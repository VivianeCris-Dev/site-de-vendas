...
function CardsCar({ car, setCar, handleRemoveCart, handleAddToCart }) {
  const total = car.reduce((acc, item) => {
    const hasOffer = item.offer != null;
    ...
    <div className="flex flex-col items-center justify-center flex-wrap gap-10 w-5/6 text-white h-full max-[817px]:text-xl">
      <div className="flex flex-wrap justify-center items-center gap-10 w-full">
        {car.map((item, index) => {
          ...
          return (
            <div
              className="flex w-2/3 h-50 items-center border rounded-md hover:scale-101 max-[1035px]:flex-col max-[1035px]:w-56 max-[1035px]:h-[420px]"
              key={index}
            >
              <img
              ...
              <div className="flex items-center justify-between p-4 w-full  max-[1035px]:flex-col">
                <div className="flex flex-col">
                  <h1 className="mb-1 text-lg">{item.name}</h1>
                  <span className="mb-2 text-gray-400 text-sm">
                    {item.description}
                  </span>
                  ...
                  {!hasOffer ? (
                    <strong className="mb-3 text-green-300">
                      Total:{" "}
                      {(finalPrice * item.quantity).toLocaleString("pt-BR", {
                        style: "currency",
                        currency: "BRL",
                      })}
                    </strong>
                    ...
                    <div className="flex flex-col mb-3">
                      ...
                      <strong className="text-green-300 text-sm">
                        Total com desconto:{" "}
                        {(finalPrice * item.quantity).toLocaleString("pt-BR", {
                          style: "currency",
                          currency: "BRL",
